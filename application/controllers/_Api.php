<?php
    class Api extends CI_Controller{
        ...
        * */
        ...
        public function signup() {
            ...
            if ($this->form_validation->run() == FALSE)
            {
                ...
                if($this->db->insert('members',$signup_data)){
                    ...
                    $firebase_token = $this->update_firebase_token($member_id,$_POST['firebase_token']);
                    // $this->sendOTP();
                    $array_res = array(
        ...
        public function edit_phone_verify(){
            $postdata = file_get_contents("php://input");
            $_POST = json_decode($postdata, true);
            $this->token_check($_POST);
        ...
        public function getAllBidsList(){
            ...
            // }else{
                ...
                // ============= adds =============
                $this->db->select('a.title,a.pic as picname,a.target');
        ...
        public function placeNewBid(){
            ...
            if ($this->form_validation->run() == FALSE)
            {
                ...
                // $this->db->select('max(amount) as minimum_amount,mb1.member_id');
                // $this->db->join('(SELECT id, MAX(amount) minimum_amount
                //                 FROM member_bids
                //                 GROUP BY id) mb1','mb.id = mb1.id AND mb.amount = mb.amount','INNER');
                // $this->db->where('mb.bid_id',$_POST['bid_id']);
                // $query_max = $this->db->get('member_bids mb');

                $this->db->select('max(mb.amount) as minimum_amount,mb.member_id');
                $this->db->where('mb.amount = (SELECT MAX(amount) FROM member_bids where bid_id = '.$_POST['bid_id'].')');
                $this->db->where('bid_id',$_POST['bid_id']);
                $query_max = $this->db->get('member_bids mb');

                // echo $this->db->last_query();

                $highest_row = $query_max->row();
                // print_r($highest_row);
                // $max_amount = $query_max->row()->minimum_amount;
                if($bid->starting_bid_amount > $highest_row->minimum_amount){
                    // echo "111";
                    $minimum_amount = $bid->starting_bid_amount;
                }else{
                    // echo "222";
                    $minimum_amount = $highest_row->minimum_amount+1;
                }

                // echo $minimum_amount;
                // exit;
                // exit;
                // echo $highest_row->member_id;
                // echo "<br/>";
                // echo $_POST['member_id'];

                if($highest_row->member_id == $_POST['member_id']){
                    $array['status'] = 320;
                    $array['title'] = 'error!';
                    $array['message'] = array('error'=> 'On this ad you have already placed highest bid ever.');
                    echo json_encode($array);
                    exit;
                }
                ...
                if(isset($minimum_amount) && $minimum_amount > $_POST['current_bid_amount']){
                    ...
                    $array['message'] = array('error'=> 'Amount Must Be Greter than '.number_format($minimum_amount).' QAR');
                    echo json_encode($array);
                    exit;
                }
        ...
        public function terms_condition_link(){
            $res = array('link' => base_url().'terms_condition');
        ...
        public function apply_coupon(){
            ...
            if ($this->form_validation->run() == FALSE)
            {
                ...
                if($_POST['amount_type'] != '0' && $_POST['amount_type'] != '1'){
                    $array['status'] = 400;
                    $array['title'] = 'Error!';
                    $array['message'] = array('error'=> 'amount_type must be 0 or 1. 0=bid_fee & 1=premium amount');
                    echo json_encode($array);
                    exit;
                }
                ...
                if($query1->num_rows() > 0) {
                    ...
                    if($coupon->users == '1'){
                        if($coupon->member_id != $_POST['member_id']){
                            $array['status'] = 320;
                            $array['title'] = 'Error!';
                            $array['message'] = array('error'=> 'The Coupon Code Not Allow For Your Account');
                            echo json_encode($array);
                            exit;
                        }
                    }

                    // if($_POST['amount_type'] == '0'){
                        if($coupon->discount_on != $_POST['amount_type']){

                            if($coupon->discount_on == '0'){
                                $amt_txt = "Bid Fee";
                            }else if($coupon->discount_on == '1'){
                                $amt_txt = "Premium Amount";
                            }

                            $array['status'] = 330;
                            $array['title'] = 'Error!';
                            $array['message'] = array('error'=> 'This Coupon Code Apply Only On '.$amt_txt);
                            echo json_encode($array);
                            exit;
                        }
                    // }
                    ...
                    if($coupon->amount_type == 0){
                        $res['total'] = $amount;
                        $res['discount'] = ($amount * $coupon->amount)/100;
                        $res['payeble_amount'] =  $amount - (($amount * $coupon->amount)/100);
                    }else if($coupon->amount_type == 1){
                        $res['total'] = $amount;
                        $res['discount'] = $coupon->amount;
                        $res['payeble_amount'] = $amount - $coupon->amount;
                    }
            ...
            if ($query1->num_rows() > 0) {
                ...
                foreach($array1 as $key_m => $val_m){
                    ...
                    if($val_m->member_notification == 'on'){
                        ...
                        if($this->db->insert('notifications',$notification_add)){
